...
#ifdef JUSTINDBG
  llvm::errs() << "in isIntegerType() with CanonicalType: \n"; CanonicalType->dump();
#endif
  if (isOverflowBehaviorType()) {
#ifdef JUSTINDBG
    llvm::errs() << "... and UnderlyingType:\n";  cast<OverflowBehaviorType>(CanonicalType)->getUnderlyingType()->dump();
#endif
    return cast<OverflowBehaviorType>(CanonicalType)->getUnderlyingType()->isIntegerType();
  }
